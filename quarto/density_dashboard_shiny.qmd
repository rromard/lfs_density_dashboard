...
# Get original variable list
orig_vars <- chart_df |> distinct(variable, var_orig, varlevel)
...
## Calculate the demographic propoertions of union members + bar chart backdrop
...
union_bars_gg <- function(.data, pal) {
  ...
  if (input$gg_measure == "Proportion") {
    ...
    gg <- ggplot(.data,
      ...
      geom_bar_text(
        formatter = number_format(accuracy = 1L, scale_cut = cut_short_scale()),
        fontface = "bold",
        outside = TRUE,
        contrast = TRUE,
        size = 15,
...
kbl_df <- reactive({kb_prep(dataset_pop())})
