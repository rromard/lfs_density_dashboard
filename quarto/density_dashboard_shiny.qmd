...
xfun::pkg_attach("tidyverse", "here", "scales", "qs", "targets",  "DT", "zoo", "patchwork", "ggiraph", "shiny", "rlang", "ggrepel", "vctrs", "ggtext")
dashpal <-  c("#8f1f3f", "#6F8FCF", "#de860b","#d4ae0b", "#3E7D49","#c73200", "#b7a4d6", "#767676", "#020101")
dens_pal <- list(c(`Union member` = dashpal[2], `Not member` = "#DADDEF"),
                      c(`Union member` = dashpal[1], `Not member` = "#DADDEF"))
dens_pal_flat <- c("#DADDEF", dashpal[2], "#DADDEF",dashpal[1])
...
union_density_gg <- function(.data, pal) {
  ...
  gg <- ggplot(
    ...
    geom_point_interactive(
      data = filter(.data, union_bin == "Union member"),
      aes(tooltip = percent(p_v, accuracy = .1), data_id = survyear),
      size = 2.25
      ) +
...
# Programmatic chart and output height (has to be in inches for ggiraph)
p_height <- reactive({5 * num_rows()})

output$plot <- renderGirafe({
  gg <- union_density_patch(dataset(), dens_pal)
  gg_i <- ggiraph:: girafe(
    ggobj = gg,
   # height_svg = p_height(),
   #  width_svg = 10,
